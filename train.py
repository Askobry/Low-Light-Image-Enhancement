...
model_restored = LLFormer(inp_channels=3,
                          out_channels=3,
                          dim = 16,
                          num_blocks = [2,4,8,16],
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        # Compute loss
        loss_l1 = L_l1(restored, target)
        loss_ssim = L_ssim(restored, target)
        loss_color = L_color(restored, target)
        loss_brightness = L_bright(restored, target)
        loss_structure = L_grad(restored, target)
        loss = loss_l1 + loss_ssim + loss_color + loss_brightness + loss_structure
